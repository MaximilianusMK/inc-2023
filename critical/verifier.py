...
def toposort(cur, adj_list, visited):
  if visited[cur] == 2:
    return
  assert visited[cur] != 1, 'cycle detected at node ' + str(cur)
  visited[cur] = 1
  for nx in adj_list[cur]:
    if visited[nx] == 2:
      continue
    toposort(nx, adj_list, visited)
  visited[cur] = 2
...
def main(argv):
  ...
  toposort(1, adj_list, visited)

  for i in range(1, N + 1):
    assert visited[i] == 2, "node " + str(i) + " disconnected"
  
  assert len(edges) == M, "multi edges detected"
